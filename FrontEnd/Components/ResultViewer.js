...
//   useEffect(() => {
//     fetch('http://192.168.42.183:8000/transcripts')
//       .then(res => res.json())
...
//     setLoading(true);
//     fetch(`http://192.168.42.183:8000/transcript/${selectedId}/analysis`)
//       .then(res => res.json())
...
//             {(Array.isArray(result.keywords) ? result.keywords : result.keywords.split(',')).join(', ')}
//           </Text>
...
//           <Text style={styles.resultText}>
//             {(Array.isArray(result.topics) ? result.topics : result.topics.split(',')).join(', ')}
//           </Text>
//         </View>
//       )}
//     </ScrollView>
...
const ResultViewer = () => {
  ...
  // Fetch list of transcripts
  useEffect(() => {
    fetch('http://192.168.45.183:8000/transcripts')
      .then(res => res.json())
      .then(data => setTranscripts(data))
      .catch(err => console.error('Failed to fetch transcript list:', err));
  }, []);
  ...
  useEffect(() => {
    if (!selectedId) return;
    setLoading(true);
    fetch(`http://192.168.45.183:8000/transcript/${selectedId}/analysis`)
      .then(res => res.json())
      .then(data => setResult(data))
      .catch(err => console.error('Failed to fetch result:', err))
      .finally(() => setLoading(false));
