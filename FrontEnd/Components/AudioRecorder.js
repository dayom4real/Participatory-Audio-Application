...
const AudioRecorder = () => {
  ...
  const enterRecordMode = () => {
    setMode('record');
    setTimeout(() => setShowRecordingUI(true), 100);
  };
  ...
  const uploadAudio = async () => {
    ...
    try {
      const response = await fetch("http://192.168.202.183:8000/upload-audio", {
        method: "POST",
        body: formData,
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {!mode ? (
          <View style={styles.modeBox}>
            ...
            <TouchableOpacity onPress={enterRecordMode} style={styles.modeButton}>
  ...
  <View style={styles.transcriptionBox}>
    <Text style={styles.label}>🗣️ Speaker Segmented Transcript</Text>
    {transcription.map((seg, index) => (
      <View key={index} style={styles.segmentBox}>
        <Text style={styles.speakerLabel}>🎙️ {seg.speaker}</Text>
        <Text style={styles.segmentText}>{seg.text}</Text>
      </View>
    ))}
    <TouchableOpacity onPress={downloadTranscription} style={styles.button}>
